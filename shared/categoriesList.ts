import {
  CategoriesListOptionsType,
  CategoriesListType,
  CategoryName,
  CategoryNameTransformed,
} from "./index";
...
//TODO keep updated
export const BroadCategories: CategoryName[] = [
...
export const CategoriesListNames = CategoriesList.map((categoriesListItem) => {
  return {
    broadCategory: categoriesListItem.broadCategory,
    categories: categoriesListItem.categories.map((category) => category.name),
  };
});

export const CategoriesListOptions: CategoriesListOptionsType[] = [];
CategoriesList.forEach((broadCategory) => {
  broadCategory.categories.forEach((category) => {
    CategoriesListOptions.push(category);
  });
});

export const transformCategoryName = (
  c: CategoryName
): CategoryNameTransformed => {
  switch (c) {
