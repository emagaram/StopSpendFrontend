...
import { ProgressTextMessage } from "shared";
import { StackedText } from "./SmallActivityRow";
export default function SmallProgressActivityRow({
  activity,
  openModal,
  className,
  timezone,
}: {
  activity: ProgressTextMessage;
  openModal: () => void;
  className?: string;
  timezone?: string;
}) {
      ...
      <td className="ps-0 pe-2 py-4">
        <Stack className="gap-sm-1" direction="horizontal">
          <StackedText
            ...
            bottomText={"Progress Text to Supporters"}
